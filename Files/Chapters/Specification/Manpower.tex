...
% Once the skillsets were documented, tasks were assigned based on the \textbf{specific requirements} of the project. \textbf{Matching} the skills of team members with the demands of each task was a strategic approach that aimed to leverage their \textbf{unique} strengths. This thoughtful allocation of responsibilities not only optimized the utilization of individual skills but also fostered a \textbf{collaborative} environment where team members could complement each other's abilities.

% The result was a well-coordinated and efficient team, where each member \textbf{contributed meaningfully} to the project. The thorough assignment of tasks according to skill-sets not only enhanced the quality of the work but also facilitated a smoother workflow, ultimately leading to the successful completion of each task within the \textbf{stipulated timelines}.

% We used \textbf{GitHub} to organise the project. We set up detailed \textbf{Workflows}, \textbf{Actions} and \textbf{Bots}, automating \textbf{.pdf} generation after new \textbf{Commits} and a thorough \textbf{Code-Structure} which made handing on task to others (who are unfamiliar with the \textbf{Codebase}), very smooth and also avoiding \textbf{Merge Conflicts}.
